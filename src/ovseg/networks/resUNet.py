...
from ovseg.networks.blocks import SE_unit, StochDepth
from ovseg.networks.custom_normalization import my_LayerNorm, no_z_InstNorm
...
class ResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv_fctn(
            ...
            self.out_channels,
            self.kernel_size2,
            padding=get_padding(self.kernel_size2),
            **self.conv_params,
        )
        ...
        if self.downsample and self.in_channels in [1, self.out_channels]:
            ...
            self.skip = nn.Sequential(
                pool_fctn(self.first_stride, self.first_stride),
    ...
    def forward(self, xb):
        xb_res = self.nonlin2(
            self.norm2(self.conv2(self.nonlin1(self.norm1(self.conv1(xb)))))
        )
        if self.use_se:
    ...
    def __init__(
        ...
        if self.is_2d:
            ...
            if norm.lower().startswith("batch"):
                ...
                norm_fctn = my_LayerNorm
        self.conv1 = conv_fctn(
            self.in_channels, self.hid_channels, 1, **self.conv_params
        )
        self.conv2 = conv_fctn(
            self.hid_channels,
            self.hid_channels,
        ...
        self.conv4 = conv_fctn(
            self.hid_channels, self.out_channels, 1, **self.conv_params
        )
        self.norm1 = norm_fctn(self.hid_channels, **self.norm_params)
...
class MergeAndRunBlock(nn.Module):
    ...
    def forward(self, xb):
        ...
        skip = 0.5 * (
            skip
            + torch.cat(
                [skip[:, self.out_channels // 2 :], skip[:, : self.out_channels // 2]],
                1,
            )
        )
        xb_res = self.nonlin2(
...
class stackedResBlocks(nn.Module):
    def __init__(
        ...
        in_channels_list = [self.in_channels] + (self.n_blocks - 1) * [
            self.out_channels
        ]
        ...
        if self.block == "res":
            res_block = ResBlock
            kwargs = {"stochdepth_rate": self.stochdepth_rate, "use_se": self.use_se}
...
class UpConv33(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv2 = nn.ConvTranspose3d(
            in_channels, out_channels, kernel_size=1, stride=(1, 2, 2), bias=False
        )
...
class UpLinear(nn.Module):
    def __init__(self, kernel_size, is_2d):
        ...
        if is_2d:
            self.up = nn.Upsample(
                scale_factor=kernel_size, mode="bilinear", align_corners=True
            )
        else:
            self.up = nn.Upsample(
                scale_factor=kernel_size, mode="trilinear", align_corners=True
            )
...
class UNetResDecoder(nn.Module):
    def __init__(
        ...
        self.filters_list = [
            min([self.filters * 2**i, self.filters_max]) for i in range(self.n_stages)
        ]
        ...
        self.z_to_xy_ratio_list = [
            max([self.z_to_xy_ratio / 2**i, 1]) for i in range(self.n_stages)
        ]
        if self.is_2d:
            self.kernel_sizes_up = (self.n_stages - 1) * [3]
        else:
            self.kernel_sizes_up = [
                (1, 3, 3) if z_to_xy >= 2 else 3 for z_to_xy in self.z_to_xy_ratio_list
            ]
        self.init_stride_list = [1] + [get_stride(ks) for ks in self.kernel_sizes_up]

        # blocks on the contracting path
        self.blocks_down = []
        for in_ch, out_ch, init_stride, z_to_xy in zip(
        ...
        block = ConvNormNonlinBlock(
            in_channels=self.in_channels_up_list[0],
        ...
        for in_channels, out_channels, n_blocks, z_to_xy in zip(
            self.in_channels_up_list[1:],
            self.out_channels_list[1:-1],
            self.n_blocks_list[1:-1],
            self.z_to_xy_ratio_list[1:-1],
        ):
            block = stackedResBlocks(
                block=self.block,
                n_blocks=n_blocks,
    ...
    def update_prg_trn(self, param_dict, h, indx=None):
        if "p_dropout_logits" in param_dict:
            p = (1 - h) * param_dict["p_dropout_logits"][0] + h * param_dict[
                "p_dropout_logits"
            ][1]
...
class UNetResStemEncoder(nn.Module):
    def __init__(
        ...
        # up to a max of filters_max
        self.n_stages = len(n_blocks_list)
        assert self.n_blocks_list[0] == 1, (
            "The stem version requires only one block on top"
        )
        self.filters_list = [
            min([self.filters * 2**i, self.filters_max]) for i in range(self.n_stages)
        ]
        ...
        self.init_stride_list = [1, 1] + [
            get_stride(ks) for ks in self.kernel_sizes_up[1:]
        ]
        ...
        self.blocks_down = []
        for n_blocks, in_ch, out_ch, init_stride, z_to_xy in zip(
            self.n_blocks_list[1:],
            self.in_channels_down_list[1:],
            self.out_channels_list[1:],
            self.init_stride_list[1:],
            self.z_to_xy_ratio_list[1:],
...
class UNetResShuffleEncoder(nn.Module):
    def __init__(
        ...
        self.in_channels_down_list = [
            self.in_channels,
            self.r_shuffle**2 * self.in_channels,
        ] + self.filters_list[1:-1]
        self.in_channels_up_list = [2 * f for f in self.filters_list[:-1]]
        self.out_channels_list = self.filters_list
        self.z_to_xy_ratio_list = [
            max([self.z_to_xy_ratio / 2**i, 1]) for i in range(self.n_stages)
        ...
        self.all_logits = [
            Logits(
                in_channels=self.out_channels_list[1],
...
class UNetResEncoder(nn.Module):
    def __init__(
        ...
        self.blocks_up = []
        for in_channels, out_channels, kernel_size in zip(
            self.in_channels_up_list, self.out_channels_list, self.kernel_sizes_up
        ):
            block = ConvNormNonlinBlock(
                in_channels=in_channels,
                out_channels=out_channels,
    ...
    def load_matching_weights_from_pretrained_model(
        self,
        data_name,
        preprocessed_name,
        model_name,
        fold,
        model_params_name="model_parameters",
        network_name="network",
    ):
        model_CV_path = os.path.join(
            os.environ["OV_DATA_BASE"],
            "trained_models",
            data_name,
            preprocessed_name,
            model_name,
        ...
        model_params = pickle.load(
            open(os.path.join(model_CV_path, model_params_name + ".pkl"), "rb")
        )
        ...
        for m1, m2 in zip(
            [self.blocks_down, self.blocks_up, self.upsamplings, self.all_logits],
        ...
        print(
            "Done! Loaded {} and skipped {} modules".format(transfered, not_transfered)
        )
...
class UNetResEncoderV2(nn.Module):
    def __init__(
        ...
        if np.isscalar(self.filters):
            self.filters_list = [
                min([self.filters * 2**i, self.filters_max])
        ...
        for in_channels, out_channels, kernel_size in zip(
            self.in_channels_up_list[j_start:],
...
class UResNet(nn.Module):
    def __init__(
        ...
        # blocks on the upsampling path
        self.blocks_up = []
        for n_blocks, in_ch, out_ch, z_to_xy in zip(
            self.n_blocks_list[:-1],
...
if __name__ == "__main__":
    net = UNetResShuffleEncoder(
        in_channels=1, out_channels=2, is_2d=False, z_to_xy_ratio=8, filters=4
    ).cuda()
